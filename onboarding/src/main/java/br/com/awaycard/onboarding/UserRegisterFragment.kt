...
class UserRegisterFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        requireView().findViewById<ScreenIndicator>(R.id.userRegisterIndicator).setOnClickListener {
            val input =
                requireActivity().findViewById<AppCompatEditText>(R.id.userRegisterInput).text.toString()

            lifecycleScope.launch {
                viewmodel.execute(input)

                findNavController().navigate(
                    UserRegisterFragmentDirections.actionUserRegisterBottomSheetToCardRegisterFragment()
                )
            }
        }
    }
